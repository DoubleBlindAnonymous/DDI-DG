...
class GmpnnNetDrugBank(nn.Module):
    def __init__(self, in_feats, edge_feats, hid_feats, rel_total, n_iter, dropout=0):
        ...
        self.dendrite1=nn.Parameter(torch.randn(self.snd_hid_feats, self.snd_hid_feats))
        ...
        self.dendrite2 = nn.Parameter(torch.randn(self.hid_feats, self.hid_feats))
    ...
    def forward(self, batch):
        ...
        new_feats = new_feats @ self.dendrite1 * new_feats
        # print("new_feats:",new_feats.size())
        ...
        pair_repr = attentions.unsqueeze(-1) * ((x_i[unique_drug_pair.edge_index[1]] @ self.i_pro) * (
                     x_j[unique_drug_pair.edge_index[0]] @ self.j_pro))

        
        pair_repr = attentions.unsqueeze(-1) * (
                (x_i[unique_drug_pair.edge_index[1]] @ self.i_pro) @ self.dendrite2 * (
                x_j[unique_drug_pair.edge_index[0]] @ self.j_pro))
        ...
        # print("pair_repr:", pair_repr.size())

        temp = scatter(pair_repr, unique_drug_pair.edge_index_batch, reduce='add', dim=0)
        # print("temp:",temp.size())
        pair_repr =temp[drug_pair_indices]
        # print("pair_repr:",pair_repr.size())
...
class GmpnnNetTwosides(GmpnnCSNetDrugBank):
    def __init__(self, in_feats, edge_feats, hid_feats, rel_total, n_iter, dropout=0):
    ...
    def forward(self, batch):
        ...
        pair_repr = attentions.unsqueeze(-1) * (
                    (x_i[drug_pairs.edge_index[1]] @ self.s_pro) * (x_j[drug_pairs.edge_index[0]] @ self.s_pro))
